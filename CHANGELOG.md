...
- **Culture-Affinität für Track-Zuordnung (Issue #146 B2):** `assignTracks` clustert jetzt auch Events ohne `lineageId`, aber mit gleichem `culture`-Feld, bevorzugt in eine gemeinsame Zeile (sofern überlappungsfrei) — Ergänzung zur bestehenden Lineage-Gruppierung. Reduziert Datenpflegeaufwand: durchgehende Themen-Spuren entstehen jetzt schon allein durch das vorhandene `culture`-Feld, ohne dass jede Nachfolgekette explizit über `lineageId` verknüpft werden muss.
