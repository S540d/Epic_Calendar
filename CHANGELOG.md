...
- **Themen-Filter, kontinentübergreifend (#226, erster Umsetzungsschritt):** Neue dritte Sektion im `FilterSheet` — 6 kuratierte, kontinentübergreifende Themen (Völkerwanderungen & Besiedlung, Kolonialismus & Eroberung, Seefahrt & Entdeckungen, Demokratiebewegungen, Aufklärung & Wissenschaft, Kalter Krieg). `src/data/themes.ts` definiert ein kontrolliertes Vokabular (`Theme = {id, labelKey, tagIds, icon}`), das auf bereits vorhandene, bereits kontinent-/kategorieübergreifend vergebene `TimelineEvent.tags`-Werte abbildet (z. B. `kolonialismus` deckt bereits Afrika, Amerika, Asien, Europa, Ozeanien ab) — kein neues Datenfeld, keine Content-Migration nötig. Anders als der Kultur-/Länder-Filter (#163) ist der Themen-Filter bewusst **nicht** kontinentgebunden und bleibt beim Kontinentwechsel aktiv; ein Such-/Lernreise-Sprung setzt ihn zurück, damit das Ziel-Event nicht unsichtbar bleibt. `theme` ist als optionaler Parameter durch `eventIndex.ts`/`culling.ts`/`TimelineView` gefädelt, analog zum bestehenden `culture`-Filter (reine Sichtbarkeits-Filterung, kein Highlight-Modus). Abgegrenzt von den Lernreisen (#171): eine Reise ist eine geführte, geordnete Stationsliste mit eigener UI, ein Thema ist ein Filter auf dem frei zoom-/pan-baren Zeitstrahl.
- **Themen-Sektion auf der Startseite (#226-Folge, Sichtbarkeit):** Der Themen-Filter aus #226 war im `FilterSheet` vergraben und praktisch unauffindbar. Neue Sektion „Themen entdecken" auf der Landing Page (`EpochOverviewScreen`, zwischen Lernreisen und Epochen-Kacheln) zeigt alle 6 Themen als Chips mit Icon, Name und Event-Anzahl. Ein Tipp aktiviert den Filter und verlässt die Übersicht (analog zum Start einer Lernreise); erneutes Tippen auf das bereits aktive Thema löscht den Filter wieder, ohne den Umweg über das `FilterSheet` — ein schneller Rückweg für Discovery-Zwecke.
- **Content-Pflege für dünne Themen (#226-Folge):** Zwei gezielte `tags`-Ergänzungen auf bereits vorhandenen Events (keine neuen Fakten, kein Migrationsrisiko) verbessern die Kontinent-Abdeckung der Themen: `eu-voelkerwanderung` (Europa) und `as-vedische-periode` – „Einwanderung der Indo-Arier" laut eigener Beschreibung (Asien) erhalten den Tag `migration` und bringen das Thema „Völkerwanderungen & Besiedlung" damit auf alle 6 Kontinente (vorher 4). `as-ghandhi-unabhaengigkeit` erhält zusätzlich den Tag `demokratie` (Indien wird nach der Unabhängigkeit 1947 zur bevölkerungsreichsten Demokratie der Welt) und bringt „Demokratiebewegungen" von 3 auf 4 Kontinente. Bewusst nicht angetastet: das „Islamische Goldene Zeitalter" wurde trotz thematischer Nähe (Wissenschaft/Philosophie) nicht dem Thema „Aufklärung" zugeordnet — die europäische Aufklärung (17./18. Jh.) ist historisch ein eigenständiger Kontext, eine Gleichsetzung wäre Datumsgenauigkeit für thematische Bequemlichkeit geopfert.
...
- **Schulwissen für „Kultur & Kunst" auf 4 Kontinenten ausgebaut (Content-Coverage-Gap):** Der `npm run coverage`-Report zeigte `kultur` bei nur 6 `core`-Events (Schulwissen-Stufe), verteilt auf 3 Kontinente — Afrika, Ozeanien und weite Teile Asiens/Amerikas hatten keine einzige. 4 neue `core`-Events schließen die Lücke und stellen Bezüge zu bereits vorhandenen Events her statt isolierte Fakten anzuhängen: Benin-Bronzen (`af-kultur-benin-bronzen`, Afrika) verweist auf das bestehende Benin-Reich und die Restitutionsdebatte; Haka und Tā Moko (`oz-kultur-haka-tamoko`, Ozeanien) verweist auf die bestehende Maori-Kultur-Zivilisation; Matsuo Bashōs Haiku-Dichtung (`as-kultur-basho-haiku`, Asien) teilt sich mit dem bestehenden Ukiyo-e-Event neu eine `lineageId` (`japan-edo-kultur`) und zeigt damit eine sichtbare Verbindungslinie zwischen beiden Edo-zeitlichen Kunstformen; Samba und Karneval (`am-kultur-samba-karneval`, Amerika) stellt textlich die Parallele zur zeitgleichen Harlem Renaissance (USA) und Négritude-Bewegung (Frankreich/Afrika) als gemeinsame afrodiasporische Strömung her. `kultur` wächst von 32 auf 36 Events, `core`-Anteil von 6 auf 10.

### Fixed

- **Themen-Filter zeigte nie alle Ereignisse eines Themas (#236):** Ein Thema ist laut Design (#226) kontinentübergreifend, der Kontinent-Filter griff beim Auswählen eines Themas aber trotzdem — sichtbar war immer nur der Anteil des aktuell gewählten Kontinents, nicht alle Ereignisse des Themas. `EventIndex.queryVisible`/`getFilteredCategory` und `culling.ts`s `filterVisible` überspringen das Kontinent-Gate jetzt vollständig, sobald `theme` gesetzt ist. Zusätzlich waren die übrigen Filter beim Aktivieren eines Themas nicht vorbelegt: `TimelineScreen.applyThemeSelection` aktiviert jetzt die Kategorien aller passenden Ereignisse (sonst blieben sie hinter einem ausgeschalteten Kategorie-Filter versteckt) und zoomt per `epochRange` auf deren volle Jahresspanne — ein Thema mit 10 Ereignissen startet jetzt in der Zoomstufe, in der alle 10 sichtbar sind, statt in einer zufälligen Ausschnittsansicht.

- **Sprung-Ziel in weit unten liegender Lane blieb unsichtbar (#171-Lernreise-Folge):** `zoomToFit` bei Such-Treffern und Lernreise-Stationen zoomte zwar horizontal auf die richtige Zeit, scrollte aber nie vertikal — lag das Ziel-Event in einer erst durch den Sprung aktivierten, weit unten liegenden Lane (typischerweise `herrscher`), blieb die Bar komplett außerhalb des sichtbaren Bereichs. Neu: `TimelineView.scrollToEventLane` scrollt die Ziel-Lane nach Abschluss der Zoom-Animation in den sichtbaren Bereich. Auf Web musste dafür `ScrollView.scrollTo()` umgangen werden — react-native-web implementiert es über die Options-Objekt-Form von `Element.scroll()`, die auf dem betroffenen Element in echtem Chromium ein stiller No-op ist; ein direktes `.scrollTop`-Setzen auf den per `getScrollableNode()` geholten rohen DOM-Node funktioniert zuverlässig.
...
- **Startseite: Lernreisen und Themen im einheitlichen Karten-Layout (#236):** Die Lernreisen-Sektion (bisher volle Breite, ein Eintrag pro Zeile) und die Themen-Sektion (bisher schmale Pill-Chips in einer umbrechenden Zeile) nutzen jetzt dasselbe 2-spaltige Karten-Grid (`EpochOverviewScreen`s neue `cardGrid`/`card`-Styles ersetzen die getrennten `journeyCard`- und `themeChip`-Styles) — eine visuelle Sprache statt zwei. Themen-Karten zeigen kein Icon mehr (das `icon`-Feld ist komplett aus `Theme`/`THEMES` entfernt, da es sonst unbenutzt geblieben wäre); Lernreisen behalten ihr Icon.
- **Event-Balken nochmals deutlich kompakter (#206-Folge):** `TRACK_HEIGHT` (64 → 40px) reduziert die sichtbare Balkenhöhe von 56px auf 32px (−43 % ggü. #207, −56 % ggü. dem Ursprungswert). Die 44px-Touch-Zielgröße bleibt dabei erhalten, ist aber jetzt von der visuellen Größe entkoppelt: die Tap-Trefferfläche wird unabhängig vertikal auf ≥44px gepolstert (`TimelineView.handleCanvasTap` prüft eine zentrierte, gepolsterte Höhe statt der reinen Balken-Bounding-Box; auf Web übernimmt `hitSlop` dieselbe Rolle) — exakt das Muster, das schon `TimelineMinimap` in #214 für ihre Hairline verwendet. Bei eng gestapelten Tracks können sich die gepolsterten Trefferflächen benachbarter Zeilen um wenige Pixel überlappen; das bestehende Popover für mehrdeutige Treffer fängt diesen Fall bereits ab (dasselbe Verhalten wie bei zeitlich überlappenden Events in derselben Spur). Nebenbei behoben: `eventLabelMaxLines` erlaubte bei Level 4 drei Zeilen — deren Textblock hätte bei den neuen kompakten Balken in die nächste Spur hineingeragt, da der Label-Container nicht clippt. Auf 2 Zeilen (Level 3–4) begrenzt, um das zu verhindern.
- **Zoom-Cluster blendet bei Inaktivität ab statt Buttons zu entfernen (#215, Gegenvorschlag zu #211):** `TimelineZoomCluster` (⌖/+/−) dimmt nach 3s Inaktivität auf ~35% Opazität und wird bei Interaktion sofort wieder voll sichtbar — die Buttons bleiben dabei gemountet und tappbar, ein einzelner Tap auf den gedimmten Cluster reicht. Aufwach-Auslöser: Canvas-Pan/Pinch/Tap (über `TimelineZoomClusterHandle.wake()`, imperativ von `TimelineScreen` aus einem echten Touch-Event-Handler aufgerufen statt über eine Prop+Effect-Kaskade), Mausrad/Ctrl-Zoom sowie Hover auf Web, und das Antippen der Buttons selbst. `prefers-reduced-motion`/`AccessibilityInfo.isReduceMotionEnabled()` schaltet ohne Animation direkt um. Während einer aktiven Lernreise bleibt der Cluster wie bisher komplett ausgeblendet. `accessibilityLabel` für Zoom-in/-out sind jetzt i18n-basiert (`zoomCluster.zoomIn`/`zoomCluster.zoomOut`, vorher hartkodiertes Englisch).
- **Minimap als Hairline unter der Zeitachse (#214, Folge von #211):** `TimelineMinimap` belegt keine eigene Chrome-Zeile mehr, sondern rückt als schmale Leiste (4px statt 8px Track-Höhe) direkt unter die `TimeAxis`, innerhalb desselben sticky-Wrappers in `TimelineChrome` — dadurch scrollt sie auf Web garantiert mit der Achse statt darunter durchzurutschen. Die Trefferfläche für Tap-to-Jump bleibt über `hitSlop` bei ≥ 24px, obwohl die sichtbare Leiste dünner wird. Tap-to-Jump, a11y-Actions (`increment`/`decrement`) und der pulsierende Such-Highlight funktionieren unverändert.
- **Breadcrumb-Zeile aufgelöst (#213, Folge von #211):** `EpochBreadcrumbBar` entfällt komplett. Ihre Zoom-Pille zeigte nur die interne LOD-Nomenklatur („Level 3"), auf die niemand reagiert, und ihre Zeitraum-Angabe duplizierte die Tick-Labels der `TimeAxis`. Die einzige eigenständige Information — der Vorfahren-Pfad oberhalb der vom `EpochBand` gezeigten Ebene (z. B. „Menschheit ›" während das Band „Antike"/„Mittelalter" zeigt) — ist jetzt ein kleiner, sticky Prefix-Chip direkt im `EpochBand` (horizontal scrollbar bei mehreren Vorfahren, tippbar wie zuvor). Ersparnis: 1 komplette Chrome-Zeile ohne Funktionsverlust.
- **Filter-Sheet statt Dauer-Chrome (#212, Folge von #211):** `FilterChipBar` (Kategorien) und der Kultur-Filter-Banner waren dauerhaft über dem Zeitstrahl eingeblendet, obwohl Filtern eine Gelegenheitshandlung ist. Beide sind jetzt in einem neuen `FilterSheet`-Bottom-Sheet zusammengeführt (analog `SettingsModal`/`SearchModal`), erreichbar über ein neues 🏷-Icon im Header von Übersicht und Zeitstrahl. Ein Badge am Icon zeigt die Abweichung vom Standard quantitativ („3/6") — nur sichtbar, wenn Kategorien oder Kulturfilter vom Default abweichen. Der Kultur-Filter (bisher nur über erneutes Antippen des aktiven Kontinent-Tabs erreichbar, #163) hat damit erstmals einen expliziten Einstieg; die Tab-Geste bleibt als Shortcut erhalten. Die Kategorie-Farblegende bleibt über `TimelineLaneLabels` erhalten, auch ohne die permanente Chip-Leiste.
- **Geringere Zeilenhöhe pro Event-Track (#206):** `TRACK_HEIGHT` (80 → 64px) und `LANE_PADDING_V` (14 → 10px) reduziert — Balkenhöhe sinkt von 72px auf 56px. Betrifft v. a. dicht bespielte Lanes mit vielen parallelen Kultur-Tracks (z. B. „Keltische Kulturen"), die dadurch ~30 % weniger vertikalen Platz beanspruchen; Lanes mit nur wenigen Tracks werden entsprechend kompakter. Touch-Ziel bleibt komfortabel über dem 44px-Minimum (iOS HIG), und dreizeilige Labels bei maximalem Zoom passen weiterhin in den Balken.
- **Skalierbare Farblogik für Spuren-Farben (#162):** Die feste 7-Werte-Palette pro Kategorie (egal ob handgepflegt oder generiert) kollidierte bereits bei den aktuellen 153 `culture`-Werten spürbar und hätte sich mit wachsendem Content weiter verschlechtert; Events ohne `culture` bekamen zudem einen quasi-zufälligen Farbton pro Event (Hash auf `id`). Ersetzt durch einen echten Formel-Generator (`src/theme/colorGeneration.ts`): feste Hue aus der Kategoriefarbe, Sättigung/Helligkeit/Hue-Jitter deterministisch aus einem Hash von `culture` — skaliert auf beliebig viele Kulturen ohne wachsende Kollisionsrate. Events ohne `culture` rendern stabil in der flachen Kategoriefarbe statt zufällig. `color`-Override bleibt unverändert vorrangig. Rein optische Änderung (keine Datenmigration) — bestehende Spuren-Farben verschieben sich sichtbar leicht innerhalb ihrer Kategoriefarbfamilie, außer bei manuell gesetztem `color`.
