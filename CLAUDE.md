...
- **Culture-Affinität für Singletons (#146 B2):** `assignTracks` bevorzugt für Events ohne `lineageId`, aber mit gesetztem `culture`-Feld, einen Track, der bereits dieselbe `culture` enthält (sofern überlappungsfrei) — Ergänzung zur Lineage-Gruppierung, damit z. B. alle „römisch"-Events auch ohne gepflegte `lineageId` in einer durchgehenden Spur landen. Reihenfolge: manuelle Overrides → Lineage-Gruppen → Culture-Affinität → restliche Singletons greedy.
