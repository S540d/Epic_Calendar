...
- **Themen-Filter, kontinentübergreifend (`theme`, Issue #226, erster Umsetzungsschritt):** Dritte Sektion im `FilterSheet` neben Kategorien/Kultur. `src/data/themes.ts` definiert ein **kontrolliertes Vokabular** (`Theme = {id, labelKey, tagIds, icon}`, 6 kuratierte Themen: Völkerwanderungen & Besiedlung, Kolonialismus & Eroberung, Seefahrt & Entdeckungen, Demokratiebewegungen, Aufklärung & Wissenschaft, Kalter Krieg) statt Freitext — löst die in #226 offen gelassene Design-Frage 1. Jedes Theme mappt auf 1-2 bereits vorhandene `TimelineEvent.tags`-Werte, die schon kontinent-/kategorieübergreifend vergeben sind (z. B. `kolonialismus` deckt bereits alle 5 Kontinente ab) — kein neues Event-Feld, keine Content-Migration. `eventMatchesTheme(event, themeId)` prüft die Schnittmenge mit `event.tags`. Anders als der Kultur-Filter ist `theme` **nicht** kontinentgebunden (löst Design-Frage 2/3: reine Sichtbarkeits-Filterung wie der Kultur-Filter, kein Highlight-Modus) und bleibt beim Kontinentwechsel aktiv; ein Such-/Lernreise-Sprung (`focusEvent`) setzt ihn dagegen zurück, damit das Sprungziel nicht durch ein aktives Thema verdeckt bleibt. `theme` ist als optionaler Parameter durch `eventIndex.ts` (`IndexQuery`/`CategoryQuery`)/`culling.ts` (`VisibilityFilter`/`LaneDataInput`)/`TimelineView` gefädelt, exakt analog zum bestehenden `culture`-Parameter. `themes.test.ts` erzwingt pro Theme ≥3 Events über ≥2 Kontinente; `i18n.test.ts` erzwingt Labels in DE+EN (gleiches Guard-Muster wie Epochen/Lernreisen). Abgegrenzt von Lernreisen (#171): eine Reise führt geordnet durch Stationen mit eigener UI, ein Thema filtert den frei zoom-/pan-baren Zeitstrahl ohne erzwungene Reihenfolge.
- **Themen-Sektion auf der Startseite (#226-Folge):** Zweiter, prominenterer Einstieg in den Themen-Filter — `EpochOverviewScreen` zeigt zwischen Lernreisen- und Epochen-Sektion eine Chip-Reihe (`themeSection.title`/`hint`) mit allen `THEMES`, je mit Icon, Name und Event-Anzahl (`ALL_EVENTS.filter(eventMatchesTheme)`, inline per `useMemo` berechnet statt durch Props gereicht — bei 605 Events × 6 Themen trivial). `TimelineScreen.handleSelectThemeFromOverview` setzt `themeFilter` und verlässt die Übersicht (Muster wie `handleStartJourney`); erneutes Tippen auf das bereits aktive Thema-Chip toggelt den Filter aus, ohne die Übersicht zu verlassen — ein Schnellweg zurück, der das `FilterSheet` umgeht.
