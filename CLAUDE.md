...
- **Kultur-/Länder-Filter innerhalb eines Kontinents (`culture`, Issue #163):** Erneutes Antippen des aktiven Kontinent-Tabs (`ContinentTabBar`s `onPressActive`) öffnet `CultureFilterModal` mit allen `culture`-Werten des Kontinents (`EventIndex.culturesForContinent`). Auswahl setzt `TimelineScreen`s `cultureFilter`-State, der als optionaler `culture`-Parameter durch `TimelineView` → `buildStableTracksByLane`/`computeLaneData` → `filterVisible`/`EventIndex.queryVisible`/`getFilteredCategory` gefädelt ist (analog zum bestehenden `continent`-Filter, gleiche Optionalitäts-Konvention). Aktiver Filter ist sichtbar über Punkt-Indikator am Kontinent-Tab plus Banner-Zeile über dem Zeitstrahl; wird bei Kontinentwechsel oder Such-Sprung automatisch zurückgesetzt (Kulturen sind kontinentgebunden). `globalEventIndex.ts` exportiert die einmalig gebaute `EventIndex`-Instanz jetzt geteilt zwischen `TimelineView` und `TimelineScreen` (vorher privat in `TimelineView`).
- **Themen-Filter, kontinentübergreifend (`theme`, Issue #226, erster Umsetzungsschritt; Filterung/Vorbelegung in #236 nachgezogen):** Dritte Sektion im `FilterSheet` neben Kategorien/Kultur. `src/data/themes.ts` definiert ein **kontrolliertes Vokabular** (`Theme = {id, labelKey, tagIds}`, 6 kuratierte Themen: Völkerwanderungen & Besiedlung, Kolonialismus & Eroberung, Seefahrt & Entdeckungen, Demokratiebewegungen, Aufklärung & Wissenschaft, Kalter Krieg) statt Freitext — löst die in #226 offen gelassene Design-Frage 1. Jedes Theme mappt auf 1-2 bereits vorhandene `TimelineEvent.tags`-Werte, die schon kontinent-/kategorieübergreifend vergeben sind (z. B. `kolonialismus` deckt bereits alle 5 Kontinente ab) — kein neues Event-Feld, keine Content-Migration. `eventMatchesTheme(event, themeId)` prüft die Schnittmenge mit `event.tags`. Anders als der Kultur-Filter ist `theme` **nicht** kontinentgebunden (löst Design-Frage 2/3: reine Sichtbarkeits-Filterung wie der Kultur-Filter, kein Highlight-Modus) und bleibt beim Kontinentwechsel aktiv; ein Such-/Lernreise-Sprung (`focusEvent`) setzt ihn dagegen zurück, damit das Sprungziel nicht durch ein aktives Thema verdeckt bleibt. `theme` ist als optionaler Parameter durch `eventIndex.ts` (`IndexQuery`/`CategoryQuery`)/`culling.ts` (`VisibilityFilter`/`LaneDataInput`)/`TimelineView` gefädelt, exakt analog zum bestehenden `culture`-Parameter — **mit einer Ausnahme (#236):** Ist `theme` gesetzt, überspringen `EventIndex.queryVisible`/`getFilteredCategory` und `filterVisible` den Kontinent-Abgleich vollständig statt ihn mit `continent` zu UNDen, sonst wäre "nicht kontinentgebunden" nur für den Fortbestand des Filters beim Kontinentwechsel wahr gewesen, nicht für die tatsächlich sichtbare Ereignismenge. `TimelineScreen.applyThemeSelection` (gemeinsam genutzt vom Startseiten-Chip und dem `FilterSheet`) aktiviert beim Setzen zusätzlich alle Kategorien der passenden Events (sonst blieben sie hinter einem deaktivierten Kategorie-Filter versteckt) und setzt `epochRange` auf deren volle `startYear`/`endYear`-Spanne, damit der Zeitstrahl direkt in einer Zoomstufe startet, die alle Treffer zeigt. `themes.test.ts` erzwingt pro Theme ≥3 Events über ≥2 Kontinente; `i18n.test.ts` erzwingt Labels in DE+EN (gleiches Guard-Muster wie Epochen/Lernreisen). Abgegrenzt von Lernreisen (#171): eine Reise führt geordnet durch Stationen mit eigener UI, ein Thema filtert den frei zoom-/pan-baren Zeitstrahl ohne erzwungene Reihenfolge.
- **Themen-Sektion auf der Startseite (#226-Folge; Layout in #236 vereinheitlicht):** Zweiter, prominenterer Einstieg in den Themen-Filter — `EpochOverviewScreen` zeigt zwischen Lernreisen- und Epochen-Sektion ein 2-spaltiges Karten-Grid (`themeSection.title`/`hint`, geteiltes `cardGrid`/`card`-Styling mit der Lernreisen-Sektion, s. u.) mit allen `THEMES`, je mit Name und Event-Anzahl (`ALL_EVENTS.filter(eventMatchesTheme)`, inline per `useMemo` berechnet statt durch Props gereicht — bei 605 Events × 6 Themen trivial). Bewusst **ohne Icon** — das `icon`-Feld wurde komplett aus `Theme`/`THEMES` entfernt, da es dann in keiner UI mehr gerendert worden wäre. `TimelineScreen.handleSelectThemeFromOverview` ruft `applyThemeSelection` (s. o.) und verlässt die Übersicht (Muster wie `handleStartJourney`); erneutes Tippen auf das bereits aktive Thema-Chip toggelt den Filter aus, ohne die Übersicht zu verlassen — ein Schnellweg zurück, der das `FilterSheet` umgeht. Die Lernreisen-Sektion direkt darüber nutzt seit #236 dasselbe `cardGrid` (vorher: volle-Breite-Karten in einer Spalte) — eine visuelle Sprache für beide Discovery-Einstiege, Lernreisen behalten dabei ihr Icon.
- **Detailgrad-Filter (`importance` verdrahtet):** Der Detailgrad (**Kinder / Schulwissen** / Standard / Alles) setzt `maxImportanceRank` als kumulativen Schwellwert in `filterVisible`/`queryVisible`. Default „Alles" (= alles sichtbar, abwärtskompatibel); Events ohne `importance` zählen als `extended`. Ergänzt den automatischen Zoom-LOD um eine manuelle Achse; persistiert als `detailLevel`. UI ist eine Segmented-Control direkt in `SettingsModal` (kein eigenständiges Component mehr — die frühere `DetailLevelSelector`-Komponente war verwaist, duplizierte dieselbe UI und wurde entfernt). Die unterste Stufe `core` = **Kinder/Schulwissen** (i18n `detailLevel.core`, EN „Kids / school basics") ist systematisch über **alle** Kategorien mit typischem Schulwissen bespielt (Stand: erdzeitalter 11, herrscher 54, nation 35, zivilisation 88, natur 46, kultur 6 core) — beim Ergänzen neuer Events dieselbe Balance halten und offensichtliches Schulwissen `importance: 'core'` geben. Die oberste Stufe `detail` (Spezialisten-Tiefe) deckt inzwischen alle sechs Kategorien ab (vorher fast ausschließlich `zivilisation`/`nation`) — `npm run coverage` zeigt die aktuelle Verteilung.
- **Geteilte Renderer-Bausteine (gegen Web/Native-Drift):** Alles, was in beiden Renderern identisch ist, liegt in eigenen Komponenten statt doppelt in den ~400-Zeilen-Dateien: `TimelineChrome` (Achsenzeile inkl. FPS-Overlay + Minimap + EpochBand + Breadcrumb) und `TimelineLaneLabels` (Label-Spalte). In `TimelineCanvasWeb`/`TimelineCanvasNative` bleibt nur, was echt divergiert: Skia-`Canvas` vs. RN-Views, der native Multi-Hit-Popover, der Web-Scroll-Wrapper und der Mausrad-Shim. **Neue gemeinsame Chrome-Elemente gehören in diese Komponenten, nicht in beide Renderer.** Web-only-Styles brauchen dort keinen Platform-Fork, solange das `Platform.select` keinen `default`-Zweig hat (dann ist es auf Native `undefined`) — so sind `position: sticky` auf Achsenzeile und Label-Spalte gelöst.
- **Zoom-Buttons fix außerhalb der ScrollView (`TimelineZoomCluster`, ⌖/+/−):** Wird **nicht** mehr von `TimelineCanvasWeb`/`TimelineCanvasNative` gerendert (dort scrollte er bei vielen aktiven Lanes mit dem Zeitstrahl-Inhalt aus dem sichtbaren Bereich heraus), sondern von `TimelineScreen` als Overlay-Sibling **über** der `canvasWrap`-`ScrollView`. `TimelineView` ist dafür mit `forwardRef` + `useImperativeHandle` gebaut und exportiert den Typ `TimelineViewHandle` (`{ zoomIn, zoomOut, jumpToToday }`); `TimelineScreen` hält einen `timelineViewRef` und ruft die Kommandos darüber imperativ auf. Neue Chrome-Elemente, die zwingend viewport-fix (nicht scroll-mitlaufend) sein müssen, gehören nach demselben Muster außerhalb der ScrollView in `TimelineScreen`, nicht in die Canvas-Renderer.
...
- **Detailgrad-Hinweis beim ersten Start (`DetailLevelPrompt`):** Zentraler, einmaliger Dialog auf der Landing Page (nicht im Zeitstrahl), der auf den Detailgrad-Regler in den Einstellungen verweist statt ihn zu duplizieren. Persistiert als `detailLevelPromptSeen` (AsyncStorage via `usePersistedState`); „Zu den Einstellungen" setzt das Flag und öffnet direkt `SettingsModal`, „Später" setzt nur das Flag. Wird als Sibling neben `EpochOverviewScreen` gerendert, sichtbar solange `!detailPromptSeen`.
...
- **Kategorie „Kultur & Kunst" (`kultur`, Issue #76):** Sechste Kategorie in `src/theme/categories.ts` (Farbe `#A85FC2`, `laneOrder: 5` = unterste Lane) für gesellschaftliche Strömungen und Kunstgeschichte. Startete rein europäisch (Barock, Wiener Klassik, Weimarer Klassik, Romantik, Biedermeier, Impressionismus, Expressionismus, Bauhaus in `europa.json`) und deckt inzwischen 5 Kontinente ab: Asien (Ukiyo-e, Persische Miniaturmalerei, Mogulmalerei), Amerika (Mexikanischer Muralismus, Harlem Renaissance, Lateinamerikanischer Boom), Afrika (Négritude, Afrobeat), Ozeanien (Papunya-Tula-Punktmalerei, Hawaiianische Kulturrenaissance) — nur `global` bleibt bewusst leer, da die Kategorie inhärent an Kontinente/Kulturkreise gebunden ist. Nicht `defaultActive` (wie `nation`/`herrscher`). **`zivilisation` vs. `nation`:** konzeptionelle Abgrenzung für neue Inhalte in `docs/event-flags.md` dokumentiert (`zivilisation` = Völker/Wanderungen, `nation` = Staatsgebilde) — bestehende Events werden nicht rückwirkend migriert.
...
- **Kinderdarstellung / bekannte Geschichten (`story`, Issue #171-Folge):** Zweites optionales Schema-Feld `story?: string` neben `mnemonic` — anders als der kurze Jahres-Merkspruch ist `story` eine kurz nacherzählte Anekdote ohne Jahresbezug. Wird im `EventDetailModal` in einer eigenen Box unterhalb von `mnemonic` angezeigt (i18n-Label `event.story`), unabhängig davon ob `mnemonic` gesetzt ist. Bewusst über mehrere Kategorien/Kontinente gestreut statt nur Europa/Antike: `natur` (Archimedes „Störe meine Kreise nicht!", Newtons Apfel, Galileis „Und sie bewegt sich doch", Franklins Drachenexperiment, Marie Curies strahlende Notizbücher — alle `continent: 'global'`), `herrscher` (Buddhas Erwachen und Dschingis Khans Pfeilbündel in Asien, Kleopatras Tod in Afrika, Alexanders Gordischer Knoten und Diogenes' „Geh mir aus der Sonne" in Europa), `zivilisation` (Caesars „Auch du, Brutus?" bei seiner Ermordung). Datumsgenauigkeit vor thematischer Bequemlichkeit: Wo die Anekdote zeitlich nicht zum bestehenden Event passt (z. B. Caesars Ermordung 44 v. Chr. vs. Rubikon-Überschreitung 49 v. Chr.; Galileis Widerruf 1633 vs. Teleskop-Entdeckungen 1609), wird ein eigenes Event angelegt statt die Story ans falsche Datum zu hängen.
- **Lernreisen / geführter Kindermodus (`learningJourneys.ts`, Issue #171-Folge):** Kuratierte, lineare Touren durch vorhandene Events. Eine `LearningJourney` ist bewusst **nur** `{ id, labelKey, descriptionKey, icon, eventIds }` — die Stationstexte kommen aus dem Event selbst (`title`/`description`/`story`/`mnemonic`), damit eine Reise keine Inhalte dupliziert und nicht auseinanderlaufen kann; der Wert liegt allein in der Kuratierung (welche ~15 der 600+ Events und in welcher Reihenfolge). `learningJourneys.test.ts` erzwingt, dass jede `eventId` existiert, keine Station doppelt vorkommt, die Reihenfolge chronologisch ist und die „Berühmte Geschichten"-Reise nur Stationen mit `story` enthält; `i18n.test.ts` erzwingt Labels in DE+EN (gleiches Muster wie der Epochen-Guard). UI: `LearningJourneyBar` (Overlay **außerhalb** der Canvas-ScrollView in `TimelineScreen`, wie `TimelineZoomCluster`) zeigt die Station statt eines Modals — ein Vollbild-Modal pro Schritt würde den Zeitstrahl verdecken, und das Sehen _wo in der Zeit_ die Station liegt ist der didaktische Punkt. Der Zoom-Cluster ist währenddessen ausgeblendet. Fortschritt: `activeJourneyId` ist Session-State (Modus), der Stationsindex wird pro Reise unter `learningJourneyProgress` persistiert (Fortsetzen von der Startseite; Abschluss der letzten Station löscht den Eintrag). Stationswechsel rufen `focusEvent` **direkt im Handler** auf — nicht per Effect auf die aktuelle Station, das flaggt `react-hooks/set-state-in-effect` als Kaskaden-Render.
- **`focusEvent(event, openDetail)` in `TimelineScreen`:** geteilter Helfer für Such-Sprünge und Lernreise-Stationen — aktiviert Kategorie/Kontinent des Ziels (sonst ist es unter den aktuellen Filtern unsichtbar), verlässt die Übersicht und setzt `jumpToEvent`. `TimelineView`s `jumpToEvent` hat dafür ein optionales `openDetail` (Default `true`); die Lernreise übergibt `false`, weil sie den Stationstext in ihrer eigenen Leiste rendert.
- **Vertikaler Lane-Scroll bei Sprüngen (`scrollToEventLane`, #171-Lernreise-Folge):** `zoomToFit` löst nur den horizontalen Zeit-Zoom aus — lag das Ziel-Event in einer weit unten liegenden Lane (z. B. `herrscher`, oft erst durch den Sprung selbst aktiviert), blieb die Bar unsichtbar, weil nie vertikal gescrollt wurde. `TimelineView.scrollToEventLane` holt das nach, ausgelöst in der `jumpToEvent`-Handhabung (Suche **und** Lernreise). Zwei Besonderheiten, beide der Mühe wert, in Code-Kommentaren nachzulesen statt zu wiederholen:
  - **Timing:** `lanes`/`laneTops` kommen aus `computeLaneData` und beschreiben den _sichtbaren Viewport_ — direkt nach dem Sprung-Request also noch den _alten_ Viewport. Der Scroll läuft daher über `scrollToEventLaneRef` (stabiler Ref-Pattern wie `zoomToFitRef`) verzögert um dieselbe `ZOOM_MODAL_DELAY_MS`, mit der auch das Detail-Modal öffnet — erst dann spiegeln die Werte den neuen (gezoomten) Viewport.
  - **Web-Scroll-Ziel:** `TimelineChrome`/`TimelineLaneLabels` sind auf Web `position: sticky` — die eigentliche Scroll-Instanz ist die äußere `ScrollView` (`TimelineScreen`s `canvasScrollRef`), **nicht** der innere `overflowY:'auto'`-Wrapper in `TimelineCanvasWeb` (der ist in der Praxis inert, da er in einem höhen-unbegrenzten `ScrollView`-Content sitzt und sein `flex:1` nichts zum Flexen hat). `ScrollView.scrollTo()` selbst darf dabei **nicht** benutzt werden: react-native-web implementiert es über `node.scroll({top,left,behavior})` — die Options-Objekt-Form von `Element.scroll`/`scrollTo`, die auf diesem Element in echtem (nicht nur Test-)Chromium ein stiller No-op ist. Stattdessen wird über `getScrollableNode()` der rohe DOM-Node geholt und `.scrollTop` direkt gesetzt (verliert die Scroll-Animation, funktioniert aber zuverlässig). Nativ ist beides unproblematisch: `TimelineChrome` ist dort normaler Layout-Fluss (nicht sticky), daher braucht `scrollToEventLane` dort zusätzlich `measureLayout` gegen die äußere `ScrollView`, und `ScrollView.scrollTo()` funktioniert nativ wie erwartet.
  - Beide Canvas-Renderer sind dafür `forwardRef<View, Props>` auf den Lanes-Container (nicht auf den inerten Web-Wrapper); `TimelineView` hält dafür einen einzigen `lanesContainerRef`, der je nach Plattform an `TimelineCanvasWeb`/`TimelineCanvasNative` durchgereicht wird.
...
- AsyncStorage-Keys: `activeCategories`, `selectedContinent`, `detailLevel`, `theme_isDark`, `i18n_language`, `showFpsMonitor`, `detailLevelPromptSeen` (alle via `usePersistedState` oder direkt AsyncStorage)
...
npm run coverage   # Content-Coverage-Report (Kategorie × Kontinent / Detailgrad, s.u.)
npm run build:web  # Expo Web-Export (GitHub Pages)
```

**Content-Coverage-Report (`npm run coverage`, `scripts/coverage.js`):** Dev-only Node-Script (kein TS-Build nötig, liest die Event-JSONs direkt), zeigt zwei Konsolen-Tabellen — Events pro Kategorie × Kontinent und pro Kategorie × Detailgrad (`importance`) —, leere Kombinationen markiert mit „·". Reines Autoring-Werkzeug ohne App-UI; die Frage „wo haben wir noch inhaltliche Lücken?" ist ein Content-Pflege-Anliegen, kein Nutzer-Feature. Kategorie-/Kontinent-Reihenfolge orientiert sich an `src/theme/categories.ts`/`src/data/schema.ts`, unbekannte Werte werden automatisch angehängt statt verworfen.
...
│   ├── TimelineChrome.tsx         # Geteilter Header-Stack: Achsenzeile + Minimap + EpochBand + Breadcrumb
│   ├── TimelineLaneLabels.tsx     # Geteilte Lane-Label-Spalte (rotierte Labels + Cluster-Badge)
│   ├── TimelineZoomCluster.tsx    # Zoom-Buttons (⌖ / + / −), gerendert von TimelineScreen außerhalb der ScrollView
...
│   ├── DetailLevelPrompt.tsx      # Einmaliger Detailgrad-Hinweis auf der Landing Page, verweist auf SettingsModal
...
└── screens/
    ├── TimelineScreen.tsx     # Haupt-Screen: Landing Page ↔ Timeline-Umschaltung + SettingsModal/DetailLevelPrompt + fixer Zoom-Cluster
...
- **`package-lock.json` kann trotz grüner CI außer Sync geraten – `deploy.yml` deckt es als einziger Workflow auf:** `ci.yml` installiert mit `npm install --no-audit --no-fund` (toleriert eine nicht-synchrone Lockfile stillschweigend), `deploy.yml` dagegen mit striktem `npm ci` (bricht bei jeder Diskrepanz zu `package.json` sofort ab). Nach der schrittweisen Expo-SDK-52→57-Migration (#221/#223) driftete die Lockfile so unbemerkt auseinander – alle PR-Checks (inkl. `testing→main`-Mergeability) blieben grün, `Deploy to GitHub Pages` auf `main` schlug aber direkt nach dem Merge mit `npm error code EUSAGE` fehl (Run #46, gefixt in #230/#231). **Vor jedem Dependency-Update/SDK-Bump lokal `rm -rf node_modules && npm ci` gegenprüfen** (nicht nur `npm install`) – das ist der einzige lokale Weg, den exakten `deploy.yml`-Installationsschritt vorab zu verifizieren.
...
_Keine offenen Issues aus der letzten Runde — #76 (Mehr Inhalte), #162 (Farblogik), #163 (Länder-/Kultur-Filter) und #171 (Lernsprüche) sind umgesetzt bzw. geschlossen. #226 (Themen-Tags): kuratiertes Themen-Vokabular, Filter im `FilterSheet` sowie eigene Startseiten-Sektion umgesetzt; mögliche Folgeschritte (Mehrfachauswahl, weitere Themen/gezielte Content-Pflege über die bisherigen Tags hinaus) bewusst nicht Teil dieses Schritts._
