...
- **Kinderdarstellung / bekannte Geschichten (`story`, Issue #171-Folge):** Zweites optionales Schema-Feld `story?: string` neben `mnemonic` — anders als der kurze Jahres-Merkspruch ist `story` eine kurz nacherzählte Anekdote ohne Jahresbezug (z. B. Archimedes' „Störe meine Kreise nicht!" beim Fall von Syrakus, Event `archimedes-tod` in `natur-wissenschaft.json`). Wird im `EventDetailModal` in einer eigenen Box unterhalb von `mnemonic` angezeigt (i18n-Label `event.story`), unabhängig davon ob `mnemonic` gesetzt ist — beide Felder können unabhängig voneinander vorkommen.
